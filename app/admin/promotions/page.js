...
export default function PromotionListPage() {
  ...
  const handleEditClick = (promo) => {
    Swal.fire({
      title: 'Are you sure?',
      text: 'Do you want to edit this promotion?',
      icon: 'question',
  ...
  const handleUpdate = async () => {
    ...
    Swal.fire({
      ...
      title: res.ok ? 'Promotion updated' : 'Update failed',
      showConfirmButton: false,
      timer: 2000,
    });
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      Swal.fire({
        ...
        position: 'top-end',
        icon: res.ok ? 'success' : 'error',
        title: res.ok ? 'Promotion deleted' : 'Delete failed',
        showConfirmButton: false,
        timer: 2000,
      });
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="space-y-8 text-gray-800">
        {promotions.map((promo) => (
          <div
            key={promo._id}
            className="relative w-full h-[330px] md:h-[300px] rounded-xl overflow-hidden shadow text-white"
          >
            {editingId === promo._id ? (
              <div className="overflow-y-auto max-h-[300px] p-4 bg-[#1a1a1a] rounded">
                {/* Form edit giữ nguyên như code cũ */}
                <div className="mb-2">
                  <label className="block text-sm font-medium text-white">Title</label>
                  <input
                    className="w-full p-2 border rounded text-white"
              ...
              <div
                className="absolute inset-0 bg-cover bg-center"
                style={{ backgroundImage: `url(${promo.img_url})` }}
              >
                <div className="w-full h-full bg-black bg-opacity-50 flex flex-col justify-between p-6">
                  <div>
                    <h2 className="text-xl md:text-2xl font-bold mb-2">{promo.title}</h2>
